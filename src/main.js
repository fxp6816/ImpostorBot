...
client.on("message", (msg) => {
  ...
  if (msg.content === "/help") {
    msg.channel.send(
      "/startRound :  starts the game, mutes all players in the channel. (Make Sure to that ll players join the voice channel before.)"
    );
    msg.channel.send(
      "/unmute: unmutes players when a death has been reported. Also give miner report on the game."
    );
    msg.channel.send("/idied: yet to implemnet");
  } else if (msg.content === "/startRound") {
    //if user in a voice channel
    console.log(
      "********************* logging msg.member.voice.channel.guild.members"
    );
